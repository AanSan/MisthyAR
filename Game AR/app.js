...
function checkAlignment() {
    ...
    // We measure how close the current Orbit camera yaw/pitch match the target angles
    // controls.getAzimuthalAngle() is camera Y-rotation, controls.getPolarAngle() is camera X-rotation
    const currentYaw = controls.getAzimuthalAngle();
    const currentPitch = controls.getPolarAngle() - Math.PI / 2; // offset polar so 0 is horizon

    const targetYaw = levelData.targetAngle.y;
    const targetPitch = levelData.targetAngle.x;

    // Calculate distance/difference
    // Wrap angles to ensure shortest path calculation
    ...
    // Calculate resonance/alignment percentage (0% to 100%)
    const maxDeviationRange = 1.0; // Angle deviation representing 0% match
    alignmentProgress = Math.max(0, 1 - (totalDeviation / maxDeviationRange));
    
    // Non-linear progress curve to make close alignment feel more snappy/responsive
    alignmentProgress = Math.pow(alignmentProgress, 3); 

    const alignmentPercentage = Math.round(alignmentProgress * 100);
    
    // Update UI elements
    document.getElementById('alignment-progress').style.width = alignmentPercentage + '%';
    document.getElementById('alignment-percentage').innerText = alignmentPercentage + '%';
...
function loadLevel(levelIdx) {
    ...
    isAligned = false;
    alignmentProgress = 0;
    document.getElementById('alignment-progress').style.width = '0%';
    document.getElementById('alignment-percentage').innerText = '0%';
    document.getElementById('alignment-progress').style.boxShadow = '0 0 10px var(--secondary)';
